...
test("header with spaces in value", () => {
  const out = parseCurlToJson(
    'curl -H "User-Agent: Mozilla/5.0 (Windows NT 10.0)" https://api.example.com'
  );
...
test("header with colon in value", () => {
  const out = parseCurlToJson(
    'curl -H "X-Time: 2025-10-25T10:30:00" https://api.example.com'
  );
...
test("POST with JSON array", () => {
  const curl =
    "curl -X POST https://api.example.com/bulk -H \"Content-Type: application/json\" -d '[1,2,3]'";
  const out = parseCurlToJson(curl);
...
test("POST with nested JSON", () => {
  const curl =
    'curl -X POST https://api.example.com/users -H "Content-Type: application/json" -d \'{"user":{"name":"Alice","address":{"city":"NYC"}}}\'';
  const out = parseCurlToJson(curl);
...
test("infers JSON from body structure", () => {
  const curl =
    "curl -X POST https://api.example.com/data -d '{\"implicit\":true}'";
  const out = parseCurlToJson(curl);
...
test("multiple -d flags concatenate", () => {
  const curl =
    'curl -X POST https://api.example.com/data -d "part1" -d "part2"';
  const out = parseCurlToJson(curl);
...
test("POST with form-encoded data", () => {
  const curl =
    'curl -X POST https://api.example.com/form -H "Content-Type: application/x-www-form-urlencoded" -d "name=John&email=john@example.com"';
  const out = parseCurlToJson(curl);
...
test("multipart form with -F", () => {
  const curl =
    'curl -X POST https://api.example.com/upload -F "name=John" -F "email=john@example.com"';
  const out = parseCurlToJson(curl);
...
test("multipart file with content type", () => {
  const curl =
    'curl -X POST https://api.example.com/upload -F "document=@/path/to/doc.pdf;type=application/pdf"';
  const out = parseCurlToJson(curl);
  assert.deepEqual(out.multipart?.[0], {
    name: "document",
...
test("-G flag moves data to query params", () => {
  const curl =
    'curl -G https://api.example.com/search -d "q=hello" -d "limit=10"';
  const out = parseCurlToJson(curl);
  assert.equal(out.method, "GET");
  assert.deepEqual(out.query, {
    q: "hello",
...
test("combines URL query with -G data", () => {
  const curl =
    'curl -G "https://api.example.com/search?existing=param" -d "new=param"';
  const out = parseCurlToJson(curl);
...
test("duplicate query keys become arrays", () => {
  const out = parseCurlToJson(
    'curl "https://api.example.com/search?tag=js&tag=node&tag=api"'
  );
...
test("username with colon in password", () => {
  const out = parseCurlToJson(
    'curl -u "user:pass:word:123" https://api.example.com'
  );
  assert.deepEqual(out.auth, {
    user: "user",
...
test("multiple cookies", () => {
  const out = parseCurlToJson(
    'curl -b "session=abc; user_id=123; token=xyz" https://api.example.com'
  );
...
test("-A user agent flag", () => {
  const out = parseCurlToJson(
    'curl -A "Custom Agent 1.0" https://api.example.com'
  );
...
test("infers POST when multipart present", () => {
  const out = parseCurlToJson(
    'curl https://api.example.com -F "file=@test.txt"'
  );
  assert.equal(out.method, "POST");
});
...
test("does not parse non-JSON as JSON", () => {
  const out = parseCurlToJson(
    'curl -X POST https://api.example.com -d "plain text"'
  );
  assert.equal(out.json, undefined);
...
test("auto-detects form with multiple -d flags without Content-Type", () => {
  const curl =
    'curl -X POST https://api.example.com/api -d "name=Alice" -d "age=25"';
  const out = parseCurlToJson(curl);
...
test("auto-detects URL-encoded form data without Content-Type", () => {
  const curl =
    'curl -X POST https://api.example.com/api -d "email=user%40example.com&name=John+Doe"';
  const out = parseCurlToJson(curl);
  assert.deepEqual(out.form, {
...
test("does not parse JSON as form even without Content-Type", () => {
  const curl =
    'curl -X POST https://api.example.com/api -d \'{"name":"John"}\'';
  const out = parseCurlToJson(curl);
